...
from doltools import branchlink, branch, apply_gecko, gecko_04write
...
{{
    ...
    def build_gecko(self, address, gecko_path, offset=None):
        os.makedirs("tmp", exist_ok=True)
		...
		*(.sdata)
        ...
        with open(gecko_path, "w") as f:
            ...
            for i in range(0, len(data), 4):
                ...
                f.write(gecko_04write(address, value))
                f.write("\n")
                address = address + 4
#              f.write("{:08x}".format(value))
#                if bob: 
#                    f.write(" ")
#                else:
#                    f.write("\n")
#                bob ^= True
            ...
            for addr, func in self.branches:
                ...
                branch(self.dol, addr, functions[func])
                self.dol.seek(addr)
                value = struct.unpack(">I", self.dol.read(4))[0]
                f.write(gecko_04write(addr, value))
                f.write("\n")
